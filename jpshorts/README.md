...
**권장: 1대 = 서버, 나머지 = 브라우저 접속.** 학습 데이터(코퍼스·규칙·북마크·작업)가
서버 한 곳에 모여서 "차곡차곡 학습" 원칙이 유지된다.

```
[PC 1 — 서버]   일본쇼츠실행.bat  ← 유일하게 git·파이썬 필요, 데이터가 여기 쌓임
    │            화면에 "다른 PC 에서: http://192.168.x.x:8787" 표시됨
    ├─ [PC 2]   일본쇼츠_접속.bat  ← 처음 한 번 서버 IP 입력, 이후 더블클릭만
    └─ [PC 3]   일본쇼츠_접속.bat     (파이썬·git 설치 불필요 — 브라우저만)
```

- 처음 서버 실행 시 Windows 방화벽 창 → **[액세스 허용]** 클릭 (사설 네트워크).
- 서버 PC 가 켜져 있어야 나머지가 접속 가능. 서버 IP 가 바뀌면 접속 PC 의
  `server_ip.txt` 를 지우고 다시 입력.
- ⚠️ 3대가 **각자** `일본쇼츠실행.bat` 을 돌리는 것도 가능하지만, 학습 데이터가
  3조각으로 갈라져 권장하지 않음 (같은 API 키 쿼터도 3배로 소모).
