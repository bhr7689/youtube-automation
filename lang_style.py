...
SEED: dict[str, dict] = {
    ...
    "ja": {
        ...
        "desc": "프로 구조(JazzNe 확정 템플릿): ① 📢 저작권·オリジナル 제작·무단사용 금지·문의 이메일 → "
                "② ✔️ 채널등록·高評価 부탁🔔 → ③ 감성 도입(爽やかな夏にぴったりの…) → ④ こんにちは！…です！ "
                "인사 + 이번 테마 고른 스토리텔링 + 「볼드 플리명」(예: **「Juicy Summer Jazz Playlist」**) → "
                "⑤ 🎧【おすすめプレイリスト】 다른 영상 2~3개 크로스프로모(제목+링크) → ⑥ 🎧【Timeline】 "
                "타임스탬프(+🔄 2回目/3回目 リピート再生 표기) → ⑦ 🔄【リピート再生の設定方法】(모바일: 톱니 "
                "→追加設定→動画をループ再生 / PC: 우클릭→ループ再生) → ⑧ © Music by … © Photo by Unsplash "
                "푸터 → ⑨ 해시태그. (간소 버전은 감성詩→[Tracklist]→//MUSIC/VISUALS/SUPPORT/COLLAB→해시태그)",
