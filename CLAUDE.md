...
- [x] 🔟 **조회수 1만+ 연구소 (단독 앱 + 무인 자동 수집)**(2026-07-24, thumbnail-title-generator-app-cwexl9):
  ...
  - 📁 **감성 폴더(컬렉션)**(2026-07-25, 같은 브랜치): "일본 쇼츠 앱처럼 비슷한 감성끼리
    폴더로 모아 그 안에서 썸네일·제목 공통점·유사성 분석" 요청 → 컬렉션 기능 이식.
    · `viral_folders.py` 신규 — 로컬 JSON(`viral_folders.json`, gitignore). 폴더 CRUD
      (create/add/remove/rename/delete) + `suggest_folders`(cluster_by_style 로 감성 자동
      제안) + `folder_common`(폴더 안 제목 승리공식 + 키워드 + **유사도%**(제목 토큰 쌍별
      자카드 평균) + **공통 단어**(≥40% 등장)). self-test 통과.
    · `viral_lab_app.py`: **📁 감성 폴더 탭** 신설(총 6탭). 🧩 탭에 "📁 이 감성 묶음을
      폴더로 저장" + "2개 이상 묶음 전부 저장" 버튼. 📁 탭: 감성별 자동 제안·저장 / 폴더 선택 →
      유사도·공통단어·키워드 공통점 + 👁 썸네일 공통 시각패턴(Vision) + 썸네일·제목 갤러리 +
      표본으로 불러오기/삭제.
    · 검증: viral_folders self-test(제안·CRUD·유사도·공통단어) + AppTest 6탭 예외0 + 폴더 렌더.
  - 🐛 **"링크와 무관한 밤·재즈 생성" 버그 수정**(2026-07-25, 같은 브랜치): 사장님이 준 링크엔
    밤 관련이 거의 없는데 생성에서 밤·재즈가 나온 문제.
    · 원인①(주): OpenAI/Gemini 키 없으면 `generate_report` 가 **고정 데모(_demo_result =
      심야서점·재즈·밤 테마)**를 결과인 척 반환 → viral_lab_app 이 그 데모 title_sets 를 렌더.
    · 원인②: SPEC 프롬프트가 음악·플레이리스트용이라 재즈·새벽 편향 주입.
    · 수정: `concept_maker.REFERENCE_GROUNDING` 신설 — build_vibe_notes 앞에 항상 주입:
      "신규 소재·테마·시간대·키워드는 오직 첨부 레퍼런스 제목/Vision 에 실제 등장한 것에서만,
      레퍼런스에 없는 테마(밤·재즈 등)를 통념으로 지어내기 금지, 음악 가정도 레퍼런스가
  ...
  - 🔖🔔 **폴더 채널 북마크 + 새 영상 알림 + 새 1만+ 자동수집**(2026-07-25, 같은 브랜치):
    ...
    · 검증: viral_channels self-test(채널집계·북마크·알림 dismiss) + AppTest 7탭 예외0 +
      알림 배지·새영상·자동수집 렌더.
  - ✨ **분석↔생성 한 탭 통합(스크롤)**(2026-07-25, 같은 브랜치): 사장님 "분석하면 바로 보이고,
    그 아래로 스크롤하며 썸네일·제목 예시까지 같은 탭에서" 요청. 생성 UI를 `render_generation
    (top, src, kp)` 함수로 추출(kp=위젯/세션 키 접두사로 위치별 충돌 방지) → 🔎 발굴·분석 탭
    분석 바로 아래 + 📁 폴더 탭 분석 바로 아래 + ✨ 생성 탭에서 각각 호출. 이제 탭 전환 없이
    분석→생성이 세로로 이어짐. 검증: AppTest 예외0 + 생성버튼·톤라디오 3곳 키충돌 없음.
  - 📊 **조회수 구간 분류(7구간)**(2026-07-26, 같은 브랜치): 사장님 지정 경계로 분류 —
    1천~3천/3천~5천/5천~8천/8천~1만/1만~3만/3만~5만/5만+. `viral_lab.py`: `VIEW_TIERS`
    (하한포함·상한미만) + `view_tier_of` + `bucket_by_view_tier`(1천 이상) + `view_tier_report`
    (구간별 count·videos·제목공식). 이 뷰만 TIER_MIN=1천까지 봄(다른 탭은 1만+). `viral_lab_app`
