...
## 앱 설계 요약 (확정본)
- 목적: **여러 장르 채널을 새로 개설** — 기존 채널 데이터가 없으므로 장르 분석이 곧 새 채널 설계도.
- 두 대등 축(분리 금지, 한 파이프라인): 🔬 **장르 분석**(승리 공식 도출) ↔ ✨ **생성**(그 공식대로 제작).
- 채널당 루프: ①장르 선택 → ②장르 분석(구간별 우승 썸네일·제목 해부) → ③레퍼런스 담기 →
  ④생성(공식+무드+내 콘텐츠, 씬 이미지 직접 투입 + 글자 코드로 얹기) → ⑤자동 채점→베스트 추천.
  채널 N개면 이 루프 N벌 독립. 알림(watcher)은 3단계 보강.
- 생성 원칙: (1) 레퍼런스 이미지를 image-to-image 로 **직접 투입**(텍스트 번역 금지, 감성 보존).
  (2) **씬(글자 없는 배경)과 제목 문구를 분리** — 문구는 앱이 코드로 또렷이 얹어 한/일 글자 깨짐 방지 +
  채널 폰트 통일. (3) 여러 우승작 공통 공식 + 체크한 무드 2~3장 합성. (4) N세트 생성→자동 채점→상위만 추천.
- 구조: **장르 = 독립 채널 프로젝트** (여러 채널 운영 대시보드)
- 조회수: 발행 **6개월 이내**, 구간 **1천/3천/5천**/1만/3만/5만/10만/20만/30만+ (9단계).
  저구간(1천~5천)은 신생 채널 초기 상승 포착용 — 일 평균 조회수(바이럴 속도) 병기해 급상승 강조.
- 분석: 장르별 썸네일(구도·각도·색·문구·배경) + 제목(감각어·상황·장르·이모지) 공식
- 일치성: 썸네일↔제목 100점 채점
- 추천: 장르 공식 + 채널 정체성으로 새 썸네일·제목 세트
- **🧺 레퍼런스 바구니**: 앱 어디서든(구간 그리드·새 영상 피드·벤치 그리드) 썸네일에 ☑️ 체크 →
  {이미지+제목+출처채널+메모} 세트로 담김(채널 프로젝트별 분리). 추천 탭에서 "바구니로 만들기" →
  체크 이미지는 concept_maker `refs` 무드 이식(색·조명·질감 85% 흡수, 구도 복제 금지), 체크 제목은
  제목 공식 참고로 주입. 캡처 업로드도 같은 바구니에 합류.
- 감시: **🔖 북마크한 채널 중 🔔 알림 ON 한 것만** RSS 체크 → 새 영상 → 자동 분석 → **카톡 알림**(GitHub Actions cron, 나에게 보내기). 채널별 2단 토글(북마크=저장, 알림=발송 선택). 알림 OFF 북마크는 조용히 목록만.
- 입력: 캡처 업로드 + YouTube 자동수집 둘 다
- 🖱️ **바탕화면 아이콘 실행**: `썸네일제목분석기실행.bat`(git pull + 의존성 + streamlit run, 전용 포트 8505,
  브라우저 자동 오픈) + 전용 `.ico`(🎬 썸네일 느낌) + `바탕화면_아이콘_설치.bat`(1회 실행 →
  .ico 단 바로가기(.lnk)를 바탕화면 생성). 기존 유튜브실행.bat 계열과 동일 UX.
