...
[★★ 제목 '형식 골격' 그대로 복제 ★★]
레퍼런스 제목을 짧게 요약·단순화하지 마라. 레퍼런스 제목의 **형식 골격을 그대로 재현**하라:
· 구분자(| · • / 등)와 섹션 개수·순서를 동일하게. 레퍼런스가 여러 칸(예: 접두어 | 상황문구 |
  브랜드명 | 태그줄)이면 신규도 **같은 칸 수·같은 순서**로 만들어라(짧은 한 줄로 줄이면 실패).
· 고정 요소(접두어 예: 𝐏𝐥𝐚𝐲𝐥𝐢𝐬𝐭, 브랜드/시리즈명, 마지막 태그줄 예: '카페음악 • 노동요 •
  드라이브 🎧', 특수 굵은글씨체 𝐀𝐁𝐂, 이모지 위치·개수)는 레퍼런스에 반복되면 신규에도 그대로 유지.
· 오직 **가운데 후킹 문구(상황·감성 카피)만** 새로 바꿔라 — 나머지 골격·브랜드·태그는 복제.
· 특수 글씨체(𝐏𝐥𝐚𝐲𝐥𝐢𝐬𝐭 같은 mathematical bold)가 쓰였으면 신규 title 에도 같은 글씨체로 써라.
· 길이도 레퍼런스와 비슷하게(레퍼런스가 길고 화려하면 신규도 길고 화려하게). thumb_text 만
  짧은 후킹으로, title 은 레퍼런스급 풀 포맷으로.
